...
#> Warning in CPL_gdalwarp(source, destination, options,
#> oo, doo): GDAL Message 1: Setting nodata to nan on band
#> 2, but band 1 has nodata at nan. The TIFFTAG_GDAL_NODATA
#> only support one value per dataset. This value of nan
#> will be used for all bands on re-opening
#> Warning in CPL_gdalwarp(source, destination, options,
#> oo, doo): GDAL Message 1: Setting nodata to nan on band
#> 3, but band 1 has nodata at nan. The TIFFTAG_GDAL_NODATA
#> only support one value per dataset. This value of nan
#> will be used for all bands on re-opening
#> Warning in CPL_gdalwarp(source, destination, options,
#> oo, doo): GDAL Message 1: Setting nodata to nan on band
#> 4, but band 1 has nodata at nan. The TIFFTAG_GDAL_NODATA
#> only support one value per dataset. This value of nan
#> will be used for all bands on re-opening
```
